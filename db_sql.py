...
def pushToDatabase(link_code: str, passcode : str, original_link: str):
    db.query(f'insert into linkBase values(%s, %s, %s)', (link_code, passcode, original_link))

def searchInDatabase(link_code: str):
    url = db.query(f'select originalLink from linkBase where id=%s', (link_code,))
    if url : return True
    else : return False

def getLink(link_code: str):
    return db.query(f'select originalLink from linkBase where id=%s', (link_code,))[0][0]

def checkPass(link_code: str):
    password = db.query(f'select passCode from linkBase where id=%s', (link_code,))[0][0]
...
def availableHandle(custom_handle : str):
    print(custom_handle)
    custom_handle_found = db.query(f'select id from linkBase where id=%s', (custom_handle,))
    if custom_handle_found : return True
    else : return False
# -----------------------------------
def submitProblem(problem_statement : str):
    db.query(f'insert into problemReported values(%s)', (problem_statement,))
